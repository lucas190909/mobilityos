      ...
      <DialogContent className="max-w-2xl">
        ...
        <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label="Nationality">
            <select className={inputCls} value={form.nationality} onChange={e => setForm({ ...form, nationality: e.target.value })}>
              {COUNTRIES_FROM.map(c => <option key={c}>{c}</option>)}
            </select>
          </Field>
          ...
          <Field label="Program / University">
            <input className={inputCls} value={form.program} onChange={e => setForm({ ...form, program: e.target.value })} placeholder="MSc Computer Science" />
          </Field>
          <Field label="Intake">
            <select className={inputCls} value={form.intake} onChange={e => setForm({ ...form, intake: e.target.value })}>
              {INTAKES.map(c => <option key={c}>{c}</option>)}
            </select>
          </Field>
          <Field label="Assigned consultant">
            <select className={inputCls} value={form.consultant} onChange={e => setForm({ ...form, consultant: e.target.value })}>
              {CONSULTANTS.map(c => <option key={c}>{c}</option>)}
            </select>
          </Field>
          <Field label="Priority">
            <select className={inputCls} value={form.priority} onChange={e => setForm({ ...form, priority: e.target.value as "Low" | "Medium" | "High" })}>
              {["Low", "Medium", "High"].map(c => <option key={c}>{c}</option>)}
            </select>
          </Field>
          <Field label="Stage">
            <select className={inputCls} value={form.status} onChange={e => setForm({ ...form, status: e.target.value as Stage })}>
          ...
          <DialogFooter className="sm:col-span-2 gap-2">
            ...
            <BtnPrimary>{editing ? "Save changes" : "Create client"}</BtnPrimary>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
export function DeleteClientButton({ clientId, onDeleted }: { clientId: string; onDeleted?: () => void }) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <BtnGhost onClick={() => setOpen(true)} className="text-destructive">Delete</BtnGhost>
      <ConfirmDialog
        open={open}
...
export function UniversityFormDialog({
  ...
  const [form, setForm] = useState<Omit<University, "id">>({
    name: "",
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (editing && university) {
      updateUniversity(clientId, university.id, form);
      toast.success("University updated");
    } else {
      addUniversity(clientId, form);
        ...
        <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="University name" required>
            ...
            <datalist id="known-unis">
              {UNIVERSITIES_LIST.map(u => <option key={u.name} value={u.name} />)}
            </datalist>
          </Field>
          <Field label="Program">
          ...
          <Field label="Status">
            <select className={inputCls} value={form.status} onChange={e => setForm({ ...form, status: e.target.value as University["status"] })}>
              {["Applied", "Accepted", "Waiting", "Rejected"].map(c => <option key={c}>{c}</option>)}
            </select>
          </Field>
          <Field label="Scholarship">
            <input className={inputCls} value={form.scholarship ?? ""} onChange={e => setForm({ ...form, scholarship: e.target.value })} placeholder="$8,000 / year" />
          </Field>
          <DialogFooter className="sm:col-span-2 gap-2">
            <BtnGhost onClick={() => onOpenChange(false)}>Cancel</BtnGhost>
            <BtnPrimary>{editing ? "Save" : "Add"}</BtnPrimary>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
export function TaskFormDialog({
  ...
  useEffect(() => {
    if (open) {
      setForm({
        title: task?.title ?? "",
        clientId: task?.clientId ?? presetClientId ?? clients[0]?.id ?? "",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        ...
        <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label="Status">
            <select className={inputCls} value={form.status} onChange={e => setForm({ ...form, status: e.target.value as "Todo" | "In Progress" | "Done" })}>
              {["Todo", "In Progress", "Done"].map(c => <option key={c}>{c}</option>)}
            </select>
          </Field>
          <DialogFooter className="sm:col-span-2 gap-2">
            <BtnGhost onClick={() => onOpenChange(false)}>Cancel</BtnGhost>
            <BtnPrimary>{editing ? "Save" : "Create"}</BtnPrimary>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
export function DeleteTaskButton({ taskId }: { taskId: string }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="text-xs text-destructive hover:underline">Delete</button>
      <ConfirmDialog
        open={open}
        onOpenChange={setOpen}
...
export function DeleteUniversityButton({ clientId, uniId }: { clientId: string; uniId: string }) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <button onClick={() => setOpen(true)} className="text-xs text-destructive hover:underline">Delete</button>
      <ConfirmDialog
        ...
        onConfirm={() => {
          deleteUniversity(clientId, uniId);
          setOpen(false);
...
export function DocumentFormDialog({
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const payload = {
      name: form.name.trim(),
      status: form.status,
      uploadedAt: form.uploadedAt || undefined,
      expiresAt: form.expiresAt || undefined,
      notes: form.notes,
    };
    if (editing && doc) {
      updateDocument(clientId, doc.id, payload);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label="Status">
            <select className={inputCls} value={form.status} onChange={e => setForm({ ...form, status: e.target.value as DocStatus })}>
              {["Missing", "Pending Review", "Approved", "Rejected"].map(s => <option key={s}>{s}</option>)}
            </select>
          </Field>
          <Field label="Uploaded">
            <input type="date" className={inputCls} value={form.uploadedAt ?? ""} onChange={e => setForm({ ...form, uploadedAt: e.target.value })} />
          </Field>
          <Field label="Expires">
          ...
          <DialogFooter className="sm:col-span-2 gap-2">
            <BtnGhost onClick={() => onOpenChange(false)}>Cancel</BtnGhost>
            <BtnPrimary>{editing ? "Save" : "Upload"}</BtnPrimary>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
export function DeleteDocumentButton({ clientId, docId }: { clientId: string; docId: string }) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <button onClick={() => setOpen(true)} className="text-xs text-destructive hover:underline">Delete</button>
